...
class HTMLResourceHandler extends ResourceHandler {
  ...
  protected function handle($app, $package_original, $resource_type, $module_name, $method_name, $parameters = null) {
    ...
    if ($package === 'webroot' && WidgetsManagement::get_widget_feeder_by_url($module_name)) {
  ...
  private function set_uis($module_name, $file) {
    ...
    if (!isset($_GET['_uis'])) {
      ...
      $uis_data = WidgetsManagement::get_uis($_REQUEST['_uis']);
  ...
  public static function find_url_uis($url) {
    ...
    $statement = $pdo->prepare("SELECT * FROM ew_pages_ui_structures,ew_ui_structures " . "WHERE ew_ui_structures.id = ew_pages_ui_structures.ui_structure_id " . "AND path LIKE ?") or die($pdo->error);
    ...
    if (!$row) {
      ...
      $statement = $pdo->query("SELECT ui_structure_id, template, template_settings " . "FROM ew_pages_ui_structures,ew_ui_structures " . "WHERE ew_ui_structures.id = ew_pages_ui_structures.ui_structure_id " . "AND path = '@DEFAULT' ");
      $statement->execute();
      $row = $statement->fetch(\PDO::FETCH_ASSOC);
    }
