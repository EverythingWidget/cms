...
class medoo
{
  ...
  protected $server = 'rdbms.strato.de';
  protected $username = 'U1390475';
  protected $password = '123456myviwo';
  // For SQLite
  protected $database_file = '';
  // Optional
  protected $port = 3306;
  protected $charset = 'utf8';
  protected $database_name = 'DB1390475';
  ...
  public function __construct($options)
  {
    try
    {
      ...
      switch ($type)
      {
        ...
        case 'mysql':$commands[] = 'SET SQL_MODE=ANSI_QUOTES';
        case 'pgsql':$dsn = $type . ':host=' . $this->server . (isset($port) ? ';port=' . $port : '') . ';dbname=' . $this->database_name;
          $commands[] = $set_charset;
          break;
        case 'sybase':$dsn = $type . ':host=' . $this->server . (isset($port) ? ',' . $port : '') . ';dbname=' . $this->database_name;
          $commands[] = $set_charset;
          break;
        case 'mssql':$dsn = strpos(PHP_OS, 'WIN') !== false ? 'sqlsrv:server=' . $this->server . (isset($port) ? ',' . $port : '') . ';database=' . $this->database_name : 'dblib:host=' . $this->server . (isset($port) ? ':' . $port : '') . ';dbname=' . $this->database_name;
      ...
      foreach ($data as $key => $value)
      {
        switch (gettype($value))
        {
          ...
          case 'array':$values[] = $this->quote(serialize($value));
            break;
          case 'integer':case 'double':case 'string':$values[] = $this->quote($value);
            break;
        }
        ...
        else
        {
          ...
          switch (gettype($value))
          {
            case 'NULL':$fields[] = $column . ' = NULL';
              break;
            case 'array':$fields[] = $column . ' = ' . $this->quote(serialize($value));
              break;
            case 'integer':case 'double':case 'string':$fields[] = $column . ' = ' . $this->quote($value);
              break;
          }
        }
      }
