...
$.fn.bic_calendar = function(options) {
  ...
  this.each(function() {
    ...
    var calendar;
    var daysMonthLayer;
    var textMonthCurrentLayer = $('<div class="title-month"></div>');
    var textYearCurrentLayer = $('<div class="title-year"></div>');
    ...
    if (typeof opts.currentDate != "undefined")
    ...
    var displayYearController = false;
    ...
    function showCalendar() {
      ...
      var daysInMonth = lastDay(month, year);

      if (currentDate)
      {
        var ds = currentDate.split("-");
        //currentDay = parseInt(ds[2]);
        month = parseInt(ds[1]) - 1;
        year = parseInt(ds[0]);
        daysInMonth = lastDay(month, year);
      }
      listListeralsWeek(daysInMonth, month, year);
      ...
      var nextMonthButton = $('<td><a href="#" class="btn btn-month-next"><i class="glyphicon glyphicon-arrow-right" ></i></a></td>');
      //event
      nextMonthButton.find("a").click(function(e) {
        e.preventDefault();
        month = (month + 1) % 12;
        if (month == 0)
          year++;
      ...
      var previousMonthButton = $('<td><a href="#" class="btn btn-month-previous"><i class="glyphicon glyphicon-arrow-left" ></i></a></td>');
      //event
      previousMonthButton.find("a").click(function(e) {
        e.preventDefault();
        month = (month - 1);
        if (month == -1) {
          year--;
      ...
      var nextYearButton = $('<td><a href="#" class="btn btn-year-next"><i class="glyphicon glyphicon-arrow-right" ></i></a></td>');
      //event
      ...
      var previousYearButton = $('<td><a href="#" class="btn btn-year-previous"><i class="glyphicon glyphicon-arrow-left" ></i></a></td>');
      //event
      ...
      if (displayYearController === true)
      {
        ...
        var yearControlTextLayer = $('<td colspan=5 class="title-cell"></td>');
      ...
      if (displayMonthController === true) {
        ...
        var monthControlTextLayer = $('<td colspan=5 class="title-cell"></td>');
        ...
        if (displayYearController !== true)
        {
          monthControlTextLayer.append(textYearCurrentLayer);
        }
      ...
      //calendar n border
      calendar = $('<div class="calendar row" id="' + calendarId + '" ></div>');
    ...
    function changeDate(month, year) {
      ...
      listListeralsWeek(lastDay(month, year), month, year);
      showMonthDays(month, year);
      checkEvents(month, year);
      markSelectedDays();
    }
     ...
     */
    function listListeralsWeek(nod, month, year)
    {
      ...
      if (currentDate)
      {
        var ds = currentDate.split("-");
        currentDay = parseInt(ds[2]);
        currentMonth = parseInt(ds[1]) - 1;
        currentYear = parseInt(ds[0]);
      }
      //alert(currentDay + " " + currentMonth + " " + currentYear);

      //alert(firstDay + " " + nod);
      if (showDays != false) {
        var capaDiasSemana = $('<tr class="days-names" >');

        var endOfMonth = false;
        var dayCounter = 1;
        var dayNameIndex = firstDay;
        while (!endOfMonth)
        {
          var codigoInsertar = $('<td class="" >');
          ...
          if (dayNameIndex <= 0) {
            ...
            codigoInsertar.addClass("first");
            dayNameIndex = 0;
          }
          if (dayNameIndex === 7) {
            //codigoInsertar += ' class="sunday last"';
            codigoInsertar.addClass("sunday last");
            dayNameIndex = 0;
          }
          if (year === currentYear && month === currentMonth && dayCounter === currentDay) {
            //codigoInsertar += ' class="sunday last"';
            codigoInsertar.addClass("current");
          }
          //codigoInsertar += ">" + dayNames[dayNameIndex] + '</td>';
          codigoInsertar.text(dayNames[dayNameIndex]);
          //alert(dayNames[counter]);
          dayCounter++;
          dayNameIndex++;
    ...
    function showMonthDays(month, year) {
      ...
      var nMonth = month + 1;
      var nMonthText = (nMonth < 10) ? "0" + nMonth : nMonth;
      //alert(nMonthText);
      ...
      var dayCode = "<tr class='days-of-month'>";
      var dayText;
      while (daysCounter <= lastDayMonth) {

        //if (currentWeekDay % 7 == 1)
        //dayCode += "<tr>";
        dayText = (daysCounter < 10) ? "0" + daysCounter : daysCounter;
        dayCode += '<td id="' + calendarId + '_' + dayText + "_" + nMonthText + "_" + year + '" data-date="' + nMonthText + "/" + dayText + "/" + year + '" ';
        //add weekDay
      ...
      daysMonthLayerString += dayCode + "</tr>";
    ...
    function checkEvents(month, year) {
      if (reqAjax != false) {
        var temp = $("<span style='font-size:13px;'> loading...</span>");
        textMonthCurrentLayer.append(temp);
        //alert(month);
        //peticio ajax
        $.ajax({
          type: reqAjax.type,
...
//alert(events.toString());
    ...
    function markEvents(month, year) {
      ...
      for (var i = 0;
        ...
        //alert(events[i].date);
        if (events[i].date.split('-')[1] == temporalMonth && events[i].date.split('-')[2].substring(0, 4) == year)
        {

          var loopDayTd = $('#' + calendarId + '_' + events[i].date.replace(/-/g, "_"));
          var loopDayA = $('#' + calendarId + '_' + events[i].date.replace(/-/g, "_") + ' a');
          //alert(events[i].date + "   OK" + calendarId + '_' + events[i].date.replace(/-/g, "_"));
      ...
      $('#' + calendarId + ' ' + '.event_tooltip div').tooltip(tooltipOptions);
      $('#' + calendarId + ' ' + '.event_popover div').popover(popoverOptions);
