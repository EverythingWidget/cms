...
<script>
   ...
   $("#{{form_id}}").on("refresh", function (e, formData)
   {
      ...
      $.post("admin-api/ContentManagement/get_content_with_label", {content_id: documentId, key: "{{comp_id}}"}, function (data) {
         ...
         if (data['result'])
            $.each(data['result'], function (i, content)
            {
               ...
               if (content.id == formData.id)
               {
                  ...
                  {
                     $.post("admin-api/ContentManagement/get_article", {articleId: content.id}, function (data)
                     {
