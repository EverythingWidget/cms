...
//  var Audios = Scope.import('html/admin/content-management/media/audios.component.html');
  ...
  function MediaStateHandler(scope, state) {
    ...
    /*System.state(handler.statePath + '/photos', function (state) {
      new Photos(Scope).create(handler, state);
    });

    /*System.state(handler.statePath + '/audios', function (state) {
      new Audios(Scope).create(handler, state);
    });*/
  }
    ...
    System.entity('ui/app-bar').subSections = [
      {
        title: 'Photos',
        state: 'photos',
        url: 'html/admin/content-management/media/photos.component.html',
