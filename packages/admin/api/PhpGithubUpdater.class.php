...
class PhpGithubUpdater {
  ...
  public function installVersion($version, $root, $tempDirectory) {
    ...
    $result = $this->moveFilesRecursive(
            $tempDirectory . DIRECTORY_SEPARATOR . $extractDir, $root
    );
   ...
   * @return boolean              execution status
   */
  public function moveFilesRecursive($source, $destination, $exclude = []) {
    $result = true;
    ...
    if (file_exists($source) && is_dir($source)) {
      ...
      foreach ($files as $file) {
        ...
        if (is_dir($source . '/' . $file)) {
          foreach ($exclude as $k => $ignore) {
            echo $ignore . ' >>> ' . $destination . $file . "    ";
  ...
  public function next_version_info($version) {
    $info = [];
    $this->getReleases();
    reset($this->releases);
    $latest = current($this->releases);
    $info['current_version'] = $version;
    $info['latest_version'] = $latest['tag_name'];
    if ($this->compareVersions($version, $latest['tag_name']) >= 0) {
      $info['is_up_to_date'] = true;
    }
    else {
      $info['is_up_to_date'] = false;
    }
