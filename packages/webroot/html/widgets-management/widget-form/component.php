    ...
    <div id="widgets-list" class="form-content">
      <div class="col-xs-12 mar-bot mt">
        ...
        foreach ($widgets_types_list as $row) {
          ...
          ?>
          <div class="text-icon"
               onclick="uisWidget.showWidgetControlPanel('<?= $widget_type ?>',<?= htmlentities($prewidget_data) ?>)">
            <h4>
              <?= $row->get_title() ?>
            </h4>
            <p>
...
//    console.log('widget parameters:', wp);
      ...
      $.post('html/webroot/widgets-management/widget-form/form.php', {
        ...
        self.vue = new Vue({
          ...
          data: {
            ...
            containerClasses: updateSizeAndLayoutUI(containerClasses),
            widgetClasses: [],
            tempClasses: [],
    ...
    function updateSizeAndLayoutUI(classes) {
      var layoutClasses = [];

      var $sizeAndLayout = $('#size-layout');
      var list = $sizeAndLayout.find('input:radio, input:checkbox');
      $.each(list, function (k, field) {
        ...
        $.each(classes, function (i, className) {
          if (value === className) {
            $v.click();
            $v.prop('checked', true);
            layoutClasses.push(classes.slice(i, 1)[0]);
          }
        });
      });
